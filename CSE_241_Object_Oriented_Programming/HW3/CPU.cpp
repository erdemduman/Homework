...
void CPU::setOption(char op){tempOp = op;}

/*this function returns the option that comes from the constructor.*/

char CPU::getOption(){return tempOp;}
...
void CPU::execute(string str){
    ...
    if((inst == "HLT")||(inst == "hlt")){
		...
		instController(registerArr, inst, reg1, reg2); 

		/*option is checked here.*/

		optionController(registerArr,inst, reg1, reg2, getOption());		
...
void CPU::instController(int *registerArr, string inst, string reg1, string reg2){

	if(inst == "MOV" || inst == "mov")
    	movf(registerArr,reg1,reg2);
    else if(inst == "ADD" || inst == "add")
    	addf(registerArr,reg1,reg2);
    else if(inst == "SUB" || inst == "sub")
        subf(registerArr,reg1,reg2);
...
void CPU::movf(int *registerArr, string reg1, string reg2){
...
void CPU::addf(int *registerArr, string reg1, string reg2){
...
void CPU::subf(int *registerArr, string reg1, string reg2){
...
void CPU::prnf(int* registerArr, string reg1){
